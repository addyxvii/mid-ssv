...
//TODO WRITE CUSTOM STAKING POOL LOGIC
...
contract StakingPool is Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    LSDETH _lsdETH = new LSDETH();
    lsdETH = LSDETH(address(_lsdETH));
  ...
  function stake() public payable {
    ...
    uint256 amount_minted = (msg.value * lsdETH.sharePrice()) / 1e18;
    lsdETH.mint(msg.sender, amount_minted);
  ...
  function unStake(uint256 _amount) public {
    lsdETH.transferFrom(msg.sender, address(this), _amount);
    uint256 _amount_to_transfer = (_amount / lsdETH.sharePrice()) * 1e18;
  ...
  function updateSharePrice(uint256 _newSharePrice) internal {
    lsdETH.changeSharePrice(_newSharePrice);
  ...
  function getShareprice() public view returns (uint256) {
    uint256 _sharePrice = lsdETH.sharePrice();
    return _sharePrice;
  }
